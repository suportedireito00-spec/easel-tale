...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (payload.tokens?.length) {
      ...
      let q = supabase.from("device_tokens").select("token,user_id,platform");
    ...
    for (let i = 0; i < rows.length; i += BATCH) {
      ...
      const results = await Promise.all(batch.map(async (r) => {
        ...
        try {
          ...
          if (isInvalid) invalidTokens.push(r.token);
    ...
    return new Response(
      JSON.stringify({ sent: ok, failed: fail, total: rows.length, invalid_removed: invalidTokens.length }),
      { headers: { ...corsHeaders, "Content-Type": "application/json" } },
    );
  } catch (e) {
