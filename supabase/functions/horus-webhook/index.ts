...
async function handleIncomingMessage(admin: any, body: any) {
  ...
  // Presença "digitando…" imediata — melhora percepção de resposta.
  evolution.sendPresence(parsed.remoteJid || parsed.from, "composing", 15000).catch(() => {});
  ...
  // Presença "digitando…" contínua enquanto o Gemini processa (renova a cada 8s).
  const presenceTarget = parsed.remoteJid || parsed.from;
  const presenceTimer = setInterval(() => {
    evolution.sendPresence(presenceTarget, "composing", 15000).catch(() => {});
  }, 8000);
  let gen: AskGeminiResult;
  try {
    gen = await askGemini(history, systemPrompt, agent, parsed.from);
  } finally {
    clearInterval(presenceTimer);
    evolution.sendPresence(presenceTarget, "paused", 1000).catch(() => {});
  }
