          ...
          .then(async (r) => {
            if (!r.ok) {
              await supabase
                ...
                .update({
                  status: "erro",
                  erro_detalhe: `Falha na extração em segundo plano (${r.status}). Tente novamente.`,
                })
                .eq("livro_tabela", livro_tabela)
                .eq("livro_id", livro_id);
            }
  ...
  try {
    ...
    const runners = Array.from({ length: Math.min(CONCURRENCY, jobs.length) }, async () => {
      while (true) {
        ...
        let out = await limparLote(lote, inicio);
        ...
        if (igualCru && lote.length > 2) {
          ...
          const [a, b] = await Promise.all([
            limparLote(lote.slice(0, mid), inicio),
            limparLote(lote.slice(mid), inicio + mid),
          ]);
          out = [...a, ...b];
        }
...
async function limparLote(paginas: string[], numeroInicial: number): Promise<string[]> {
  const sys = `Você é um revisor editorial de livros jurídicos em Markdown. Para cada página, devolva a MESMA página em Markdown aplicando esta limpeza:

REMOVER (não deixar rastro):
- Números soltos de página (linhas contendo apenas "3", "27", "128" etc. — geralmente no rodapé)
- Cabeçalhos/rodapés repetidos (nome do livro, autor, capítulo em maiúsculas isolado no topo/fundo)
- Marcadores de OCR tipo "---4 |", "---27 |", "----5", pipes soltos no começo de linha
- Marcações espúrias de OCR (traços soltos "—", cifras "$" fora de contexto)
- Linhas com apenas pontuação ou caracteres decorativos

CORRIGIR:
- Palavras hifenizadas quebradas em fim de linha (junte: "adminis-\\ntrativo" → "administrativo")
- Espaços duplos, quebras de linha erradas dentro de um parágrafo
...
- NUNCA use níveis de heading maiores que ### (nada de #### ou #####). Se for subtítulo interno, use ### apenas.
