...
async function generateCover(apiKey: string, prompt: string): Promise<Uint8Array | null> {
  // O prompt já vem pronto de buildCoverPrompt (estilo oficial: fundo preto +
  // personagem + vestígios da cor da categoria). Nada é sobrescrito aqui.
  const strict = prompt;
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    ...
    if (!bytes) continue;
    lastBytes = bytes;
    const light = await isBackgroundLight(bytes);
    if (!light) return bytes;
    console.warn(`cover attempt ${attempt + 1}: rejected (light background), retrying…`);
  }
  // Nunca devolve null se houve ao menos 1 byte gerado — preferimos capa fraca a
  // um fallback bundled repetido para todos os posts.
  if (lastBytes) {
    console.warn("cover: aceitando último resultado apesar do filtro de luminosidade");
    return lastBytes;
  }
  return null;
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (specificTemaId) {
      ...
      // Preferência 1: tema cuja categoria ainda não saiu hoje
      tema = lista.find((t: any) => !catsUsadasHoje.has(t.categoria)) || lista[0];
      if (tema && catsUsadasHoje.has(tema.categoria) && lista.length > 1) {
        console.log("blog-runner: todas as categorias já usadas hoje; caindo no primeiro pendente");
      }
    }
