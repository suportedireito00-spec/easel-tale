  ...
  const tocItems = useMemo(() => {
    if (capitulos && capitulos.length > 0) {
      return capitulos.map((c, i) => ({
        nivel: 1 as number,
        titulo: (c.titulo || `Capítulo ${c.numero ?? i + 1}`).trim(),
        chapterIdx: i,
      }));
    }
    // fallback com sumario_json bruto
    return (sumario || []).map((s, i) => ({
      nivel: s.nivel,
      titulo: (s.titulo || '').replace(/\*+/g, '').replace(/^#+\s*/, '').replace(/_{2,}/g, '').trim(),
      ...
      {isDesktop && status === 'pronto' && currentPage && (
        ...
        >
          <div className="flex flex-col items-center gap-2 px-2 pt-3">
            ...
            <button
              onClick={async () => {
                const { speakNative, stopNativeSpeech } = await import('@/lib/nativeTts');
                if (speaking) {
                  await stopNativeSpeech();
                  setSpeaking(false);
                  return;
                }
                const raw = (currentPage?.md || '')
                  .replace(/<!--[\s\S]*?-->/g, '')
                  .replace(/[#*_>`~[\]()]/g, '')
                  .replace(/\s+/g, ' ')
                  .trim();
                if (!raw) { toast.info('Página sem texto para ler.'); return; }
                setSpeaking(true);
                try { await speakNative(raw, { lang: 'pt-BR' }); } finally { setSpeaking(false); }
              }}
              aria-label={speaking ? 'Parar leitura' : 'Ouvir página'}
              title={speaking ? 'Parar leitura' : 'Ouvir página'}
              className={`w-12 h-12 rounded-full flex items-center justify-center transition active:scale-95 ${speaking ? 'bg-primary text-primary-foreground' : dark ? 'bg-white/[0.06] hover:bg-white/10' : 'bg-black/[0.04] hover:bg-black/10'}`}
            >
      ...
      <AnimatePresence>
        {status === 'pronto' && currentPage && !menuOpen && !isDesktop && (
          <motion.div
            ...
            className="fixed z-[1310] inset-x-0 bottom-0 border-t backdrop-blur-xl shadow-2xl"
            style={{
              ...
              maxWidth:
                typeof window !== 'undefined' && window.innerWidth >= 768
                  ...
                  : undefined,
              background: dark ? 'rgba(0,0,0,0.78)' : `${tema.bg}ee`,
              borderColor: tema.border,
              color: tema.text,
            }}
          >
            ...
            <div className="flex items-center justify-around px-2 pb-4 pt-2 gap-1">
              ...
              <button
                ...
                aria-label={speaking ? 'Parar leitura' : 'Ouvir página'}
                className={`w-14 h-14 rounded-full flex items-center justify-center transition active:scale-95 ${speaking ? 'bg-primary text-primary-foreground' : dark ? 'bg-white/[0.06] hover:bg-white/10' : 'bg-black/[0.04] hover:bg-black/10'}`}
              >
