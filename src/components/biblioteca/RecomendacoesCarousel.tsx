...
const RecomendacoesCarousel = ({ onAbrirLivro }: Props) => {
  ...
  // ou pointer-capture engulam o click nativo).
  const tapRef = useRef<{ pointerId: number; x: number; y: number; livroKey: string } | null>(null);
      ...
      >
        ...
        >
          {displayList.map((livro, i) => {
            ...
            const openThis = () => onAbrirLivro(livro);
            return (
              <button
                key={cardKey}
                data-cover-item
                type="button"
                onPointerDown={(e) => {
                  tapRef.current = { pointerId: e.pointerId, x: e.clientX, y: e.clientY, livroKey: cardKey };
                }}
                onPointerUp={(e) => {
                  ...
                  if (dx > 8 || dy > 8) return; // foi arrasto, não tap
                ...
                onClick={(e) => {
                  // Fallback: se por acaso o pointerup não abriu (ex.: teclado/Enter),
                  // ainda garantimos abertura via click.
