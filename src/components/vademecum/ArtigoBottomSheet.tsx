...
const ArtigoBottomSheet = ({ artigo, onClose, isFavorito, onToggleFavorito, showNomenJuris = false, tabelaNome, forceShowRedacao, modificationInfo, breadcrumb }: ArtigoBottomSheetProps) => {
  ...
  const persistMagicHighlights = useCallback(async (grifos: MagicGrifo[]) => {
    ...
    try {
      ...
      if (notasRows.length > 0) {
        ...
        if (!notesInsertError) setAnotacoesCount((count) => count + notasRows.length);
      }
    } catch (err) {
  ...
  useEffect(() => {
    ...
    return () => { cancelled = true; };
  }, [artigo?.id, tabelaNome, showAnotacoesSheet, magicHighlights.length]);
    ...
    <>
      <Sheet open={Boolean(artigo)} onOpenChange={(open) => { if (!open) handleSheetClose(); }}>
        <SheetContent
          ...
          className={
            isDesktop
              ? "z-[9999] flex min-h-0 flex-col gap-0 overflow-hidden overscroll-contain rounded-2xl border border-white/5 bg-[#0f0f0f] p-0 shadow-2xl [&>button:last-child]:hidden top-[3%] bottom-[3%] inset-x-0 mx-auto max-w-[860px] h-[94vh]"
              : "z-[9999] flex min-h-0 flex-col gap-0 overflow-hidden overscroll-contain rounded-t-3xl border-t border-white/5 bg-[#0f0f0f] p-0 [&>button:last-child]:hidden top-8"
          }
