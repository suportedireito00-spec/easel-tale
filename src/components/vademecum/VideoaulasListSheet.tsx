...
const VideoaulasListSheet = ({ open, onClose, tabelaNome, artigoNumero, leiNome, onSelectVideo }: Props) => {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        // Resolve friendly lei name to improve YouTube search relevance
        let friendlyLei = leiNome;
        if (!friendlyLei || friendlyLei === tabelaNome) {
          try {
            const { getLeisCatalog } = await import('@/services/legislacaoService');
            const cat = getLeisCatalog?.() || [];
            const found = cat.find((l: any) => l.tabela_nome === tabelaNome);
            if (found?.nome) friendlyLei = found.nome;
          } catch { /* ignore */ }
        }
        const { data, error: fnErr } = await supabase.functions.invoke('buscar-videoaulas', {
          body: { tabelaNome, artigoNumero, leiNome: friendlyLei || tabelaNome },
        });
        if (cancelled) return;
        if (fnErr) throw fnErr;
        let list = (data?.videos || []) as VideoaulaItem[];

        // Reordena pelas curtidas do app (mais likes = mais recomendado)
        try {
          ...
          if (ids.length > 0) {
            ...
            (stats || []).forEach((s: any) => map.set(s.video_id, (s.likes_count || 0) - (s.dislikes_count || 0)));
