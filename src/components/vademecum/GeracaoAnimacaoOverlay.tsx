...
// Vozes alternadas M/F contagiantes
const VOZES_ALT = ["Puck", "Sulafat", "Fenrir", "Aoede", "Algenib", "Autonoe"];

// Cache em memória de áudios já baixados (por texto+voz)
const audioCache = new Map<string, string>();
const inflight = new Map<string, Promise<string | null>>();

function vozFor(frase: FraseOverlay, rotation: number): string {
  return frase.voz_preferida || VOZES_ALT[rotation % VOZES_ALT.length];
}

function prefetchAudio(frase: FraseOverlay, voz: string): Promise<string | null> {
  const key = `${voz}::${frase.texto}`;
...
export const GeracaoAnimacaoOverlay = ({
  ...
  useEffect(() => {
    ...
    carregarFrases().then((f) => {
      ...
      setQuoteIdx(start);
      // Pré-aquece as próximas 2 narrações para começar instantâneo
      const first = f[start];
      const second = f[(start + 1) % f.length];
      if (first) prefetchAudio(first, vozFor(first, 0));
      if (second) prefetchAudio(second, vozFor(second, 1));
    });
  }, [open]);
  ...
  // Toca áudio para a frase atual e SÓ avança quando terminar
  useEffect(() => {
    if (!open || frases.length === 0) return;
    const frase = frases[quoteIdx];
    if (!frase) return;

    const voz = vozFor(frase, rotationRef.current);
    const cacheKey = `${voz}::${frase.texto}`;

    let cancelado = false;
    setAudioProgress(0);

    // Fallback: se áudio não carregar em ~1.2s ou falhar, avança em 8s
    const armarFallback = (ms: number) => {
    ...
    const tocar = async (url: string) => {
      ...
      const updateProgress = () => {
        if (cancelado || !audioRef.current) return;
        const dur = audioRef.current.duration;
        if (dur && isFinite(dur) && dur > 0) {
          setAudioProgress(Math.min(1, audioRef.current.currentTime / dur));
        }
        if (!audioRef.current.paused && !audioRef.current.ended) {
          raf = requestAnimationFrame(updateProgress);
        }
      };
      a.addEventListener('play', () => { raf = requestAnimationFrame(updateProgress); });
      a.addEventListener('ended', () => {
        cancelAnimationFrame(raf);
        setAudioProgress(1);
        if (fallbackTimerRef.current) clearTimeout(fallbackTimerRef.current);
        // Pequena pausa entre frases
        window.setTimeout(() => { if (!cancelado) avancarFrase(); }, 350);
      });
    ...
    (async () => {
      const url = await prefetchAudio(frase, voz);
      if (cancelado) return;
      if (!url) {
        armarFallback(1200);
        return;
      ...
      tocar(url);
      // Pré-aquece a próxima frase enquanto esta toca
      const nextIdx = (quoteIdx + 1) % frases.length;
      const nextFrase = frases[nextIdx];
      if (nextFrase) {
        const nextVoz = vozFor(nextFrase, rotationRef.current + 1);
        prefetchAudio(nextFrase, nextVoz);
      }
    })();
