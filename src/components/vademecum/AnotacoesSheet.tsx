  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const merged = mergeNotes((notesResult.data || []) as Anotacao[], notesFromHighlights(highlightsResult.data?.highlights));
        setNotas(merged);
  ...
  const deleteNote = useCallback(async (nota: Anotacao) => {
    if (nota.source === 'highlight') {
      ...
      const updated = Array.isArray(data?.highlights)
        ? data.highlights.filter((item: SavedHighlight) => item.id !== nota.highlightId)
        : [];
      const { error } = await supabase
        .from('artigos_grifos')
        .update({ highlights: updated, updated_at: new Date().toISOString() })
        .eq('user_id', userId)
        .eq('tabela_codigo', tabelaNome)
        .eq('numero_artigo', artigoNumero);
