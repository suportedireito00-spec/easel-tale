      ...
      {TABS.map(({ id, label, Icon }) => {
        ...
        return (
          <button
            ...
            onClick={() => onChange(id)}
            className={`group relative flex h-9 min-w-0 items-center justify-center gap-1 rounded-full px-1.5 text-[12px] font-medium transition-all sm:h-10 sm:gap-1.5 sm:px-3 sm:text-sm ${
              active
                ? 'bg-[#FEF7B8] text-foreground shadow-[0_1px_0_0_rgba(0,0,0,0.04)] ring-1 ring-[#EADF7A]/60'
