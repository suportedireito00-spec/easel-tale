...
export async function fetchMeuEspacoFeed(userId: string): Promise<MeuEspacoFeed> {
  ...
  // Contadores para o topo do Meu Espaço
  const recentesLeis = getRecentes();
  const leisDistinct = new Set<string>();
  recentesLeis.forEach((r) => leisDistinct.add(String(r.leiId)));
  (favArtigos.data ?? []).forEach((r: any) => leisDistinct.add(String(r.tabela_codigo || '')));
